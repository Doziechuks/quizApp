...
import { selectAmountOfQuestion, selectFinalScore, selectQuestionCategory, selectQuestionDifficulty, selectQuestionType } from '../redux/selector';
...
const QuestionsPage = (props) => {
  const { questionCategory, questionDifficulty, questionType, amountOfQuestion, finalScore, setFinalScore } = props;
        ...
        <div className={classes.options}>
          {
            options && options.map((option, id) => {
              return <CustomButton key={id} onClick={handleClickAnswer} isQuestion>{ decode(option) }</CustomButton>
            })
          }
        </div>
      </div>
      <p className={classes.score}>score: {finalScore} / {amountOfQuestion}</p>
    </div>
  );
}
